...
while referee.step(timestep) != -1 and SPENT_TIME < MAXIMUM_TIME:
    ...
    if len(poi_string_list) != 22:
        print('warning: manager/supervisor is expecting for ee coords. Please review your participant_controller')
        continue
    ...
    label += 'Time spent: ' + str(SPENT_TIME/1000) + '\n'
    points = point_value
    referee.setLabel(1, label, 0.15, 0.55, 0.05, 16777215, 0)

    if points == 120:
        points += max(0, MAXIMUM_TIME - SPENT_TIME)/10000
        break
